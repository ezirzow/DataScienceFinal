...
playerRankings <- transmute(year, player, tm, pos, PointsRank=(((pts/g)/20)*.25), ReboundsRank=(((trb/g)/10)*.2), AssistsRank=(((ast/g)/5)*.2), StealsRank=(((stl/g)/1.5)*.1), BlocksRank=(((blk/g)/1)*.1), TurnoversRank=(((tov/g)/3)*.1), PctRank=(((fg_pct)/.4)*.05), OverallRank=PointsRank+ReboundsRank+AssistsRank+StealsRank+BlocksRank-TurnoversRank+PctRank)%>%top_n(15)%>%arrange(desc(OverallRank))
